...
if [[ $? -eq 0 ]]; then
	set -e
	echo
	echo "Flutterwave WooCommerce is installed and active"
...
cli wp core install \
	--path=/var/www/html \
	--url=$SITE_URL \
	--title=${SITE_TITLE-"WooCommerce Flutterwave Developement"} \
...
cli wp option set woocommerce_store_address "333 Fremont Street"
cli wp option set woocommerce_store_address_2 "#999"
cli wp option set woocommerce_store_city "San Francisco"
cli wp option set woocommerce_default_country "US:CA"
cli wp option set woocommerce_store_postcode "94105"
