...
class Test_Flutterwave_Signoz_Logger extends \WP_UnitTestCase {
    ...
    public function test_api_key_constant_is_set(): void {
        $this->assertIsString( Flutterwave_Signoz_Logger::API_KEY );
    }

    public function test_no_requests_sent_when_api_key_is_placeholder(): void {
        $this->mock_merchant_api( 'Merchant' );
        $this->logger->init( 'FLWPUBK-test-key' );
        $this->capture_signoz_requests();

        // Simulate the unsubstituted build placeholder.
        $ref = new ReflectionClassConstant( Flutterwave_Signoz_Logger::class, 'API_KEY' );
        // We cannot override a class constant at runtime, so instead assert the guard
        // behaviour via the placeholder string check in send() by verifying that a
        // freshly-reset instance whose constant equals the placeholder fires nothing.
        // If API_KEY has already been substituted in this environment, skip this test.
        if ( '%%SIGNOZ_API_KEY%%' !== Flutterwave_Signoz_Logger::API_KEY ) {
            $this->markTestSkipped( 'API_KEY has already been substituted; placeholder guard cannot be tested.' );
        }
